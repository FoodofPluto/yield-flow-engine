...
# --- parse & filter ---
# Normalize noisy lines before parsing
$normalized = $engineOut |
  ForEach-Object { $_.ToString() } |
  ForEach-Object { $_ -replace '^\s*-\s+', '' } |                                   # strip bullet "  - "
  ForEach-Object { $_ -replace '^\[\d{4}-\d{2}-\d{2}T[^\]]+\]\s*\[raw\]\s*', '' } | # strip "[...][raw] "
  ForEach-Object { $_ -replace '^\[raw\]\s*', '' }                                   # strip "[raw] "

# Drop headers/rulers/empties
$clean = $normalized | Where-Object {
  $_ -and ($_ -notmatch '^\s*Name\s+APY%') -and ($_ -notmatch '^\s*-{10,}\s*$')
}

# Primary parse via helper/fallback
$rows = @($clean | Parse-EngineTable)
Write-Verbose "[parse] parsed rows: $($rows.Count)"

# Inline regex fallback if nothing parsed
if ($rows.Count -eq 0) {
  Write-Verbose "[parse-fallback] trying inline regex"
  $rxA = '^\s*(?<name>.+?)\s{2,}(?<apy>[-\d,\.]+)%?\s+(?<source>\S+)\s+(?<tvl>[\$\d,\.]+)\s+(?<chain>\S+)\s*$'
  $rxB = '^\s*(?<apy>[-\d,\.]+)%?\s{2,}(?<tvl>[\$\d,\.]+)\s+(?<chain>\S+)\s{2,}(?<name>.+?)\s*$'
  $rows = @(
    $clean | ForEach-Object {
      if ($_ -match $rxA) {
        [pscustomobject]@{
          ...
          APY    = ([double]::Parse(($matches.apy -replace ',',''), [System.Globalization.CultureInfo]::InvariantCulture))
          TVL    = ([double]::Parse(($matches.tvl -replace '[\$,]',''), [System.Globalization.CultureInfo]::InvariantCulture))
          Chain  = $matches.chain.Trim()
  ...
  Write-Verbose "[parse-fallback] parsed rows: $($rows.Count)"
}
